...
export const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (
            !email || !firstName || !surname || !normal || !management || !admin || !organisationalUnitCode ||
            !divisionCode || !password
        ) {
            res.status(403).send({'error': 'Please complete all fields.'});
        }
        ...
        const user = await User.findOne({email: email})
        if (user) {
            res.status(403).send({'error': 'The email already exists.'});
        } else {
            try {
